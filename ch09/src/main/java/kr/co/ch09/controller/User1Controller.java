...
public class User1Controller {
    ...
    // @ResponseBody는 method의 반환값을 응답객체 본문으로 출력하기 위한 어노테이션, 자동 JSON 변환 출력
    @ResponseBody
    @GetMapping("/user1")
    ...
    public User1DTO user(@PathVariable("uid") String uid) { // @PathVariable는 주소 파라미터 값을 바인딩하기 위한 어노테이션
        return user1Service.findByUid(uid);
    }
    
    /*
        @RequestBody 
        - 요청본문 내용에 포함된 데이터를 Java 객체로 변환하는 어노테이션, JSON 데이터를 수신
    
        Valid 
        - REST API 서비스 특성상 클라이언트에서 유효성검사를 하기 어렵기 때문에 백엔드에서 유효성검사를 수행
        - 수신 처리되는 DTO의 각 속성데이터를 유효성검사 하기위해 어노테이션(@NotBlank, @NotEmpty, @NotNull, @Email...)을 사용
    ...
    */
    ...
    public ResponseEntity<User1DTO> register(@Valid @RequestBody User1DTO user1DTO) { 
        ...
        // ResponseEntity 응답객체를 반환하면 @ResponseBody 어노테이션 생략 가능
    ...
    public ResponseEntity<User1DTO> modify(@RequestBody User1DTO user1DTO) {
        ...
        // ResponseEntity 응답객체를 사용하면 요청결과에 대한 다양한 사용자 정의가 가능하기 때문에 개발의 유연성이 높음
    ...
    public ResponseEntity<String> delete(@PathVariable("uid") String uid) {
        ...
        log.info("User deleted: {}", isDeleted);
